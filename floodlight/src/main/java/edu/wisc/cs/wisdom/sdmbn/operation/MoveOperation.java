...
import edu.wisc.cs.wisdom.sdmbn.apps.testing.TestTimedMoveAll;
...
public class MoveOperation extends Operation 
{
	...
	protected static Logger log = LoggerFactory.getLogger(MoveOperation.class.getSimpleName());
	...
	{
		log.debug("Issue get called in move operation");
    ...
    private void receiveStateMessage(StateMessage msg)
    {
		...
		log.debug("Printing dst src");
		log.debug(this.src + " --- " + this.dst + " --- " + this.key + " --- " + this.inPort);
