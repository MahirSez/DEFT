...
FLOOD = EthAddr('ff:ff:ff:ff:ff:ff')
...
class Tutorial(object):
	...
	def __init__(self, connection):
		...
		# self.mac_to_port = {STAMPER_MAC: 4}
		self.mac_to_port = {}
	...
	def act_like_switch(self, packet, packet_in):
		...
		print(switchName + " : " + str(packet.src) + " --> " + str(packet.dst))
		...
		if packet.dst in self.mac_to_port or (packet.src == CLIENT_MAC and packet.dst != FLOOD):

			if packet.src == CLIENT_MAC and packet.dst != FLOOD:
				print('Bypassing through stamper')
				print(str(packet.src) + " --> " + str(packet.dst))
