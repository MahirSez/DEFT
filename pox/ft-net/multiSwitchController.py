...
macToName = config.MAC_TO_HOST
macToName['ff:ff:ff:ff:ff:ff'] = 'flood'
...
class Tutorial(object):
	...
	def __init__(self, connection):
		...
		self.switchName = 'switch-' + str(self.switchID)
		switchID += 1
	...
	def act_like_switch(self, packet, packet_in):
		...
		if packet.dst in self.mac_to_port:
			...
			if self.switchID == 2 and packet.src == CLIENT_MAC and packet.type != packet.ARP_TYPE and dst_name in config.PRIMARY_TO_SECONDARY:
				...
				msg.actions.append(of.ofp_action_output(port=secondary_port))
